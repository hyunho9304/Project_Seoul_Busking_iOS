...
class ViewController: UIViewController , NMapViewDelegate , NMapPOIdataOverlayDelegate , NMapLocationManagerDelegate {
    ...
    func getMemberRepresentativeBoroughData() {
        ...
        memberInfo?.member_nickname = "가입요오"
        
        
        Server.reqMemberRepresentativeBorough(member_nickname: "가입요오") { ( memberRepresentativeBoroughData , rescode ) in
    ...
    func naverMapSetting() {
        ...
        if let mapView = navermapView {
            ...
            //  네이버 앱 키 설정
            mapView.setClientId("rmYoiNp75poYlNzAvgiM")
